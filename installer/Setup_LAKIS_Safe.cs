...
internal sealed class SafeSetupForm : Form
{
    ...
    internal SafeSetupForm()
    {
        ...
        var copyright=new Label{Left=43,Top=399,Width=335,Height=18,Text="© 2026 Luke Jeong. All rights reserved. · LAKIS v7.2.4",ForeColor=Color.FromArgb(104,112,137),Font=new Font("Segoe UI",8F)};
...
{
    private const string Revision = "v7.2.4";
    ...
    internal static void Install(string target, bool includeAnimeSharp, Action<string> report)
    {
        ...
        try
        {
            ...
            File.WriteAllText(Path.Combine(target,"VERSION"),"7.2.4");File.WriteAllText(Path.Combine(target,"install.complete"),DateTime.UtcNow.ToString("O"));File.WriteAllLines(Path.Combine(target,"network-install.log"),log.ToArray());if(previous!=null)try{DeleteTree(previous);}catch{status("이전 설치 폴더는 재부팅 후 삭제할 수 있습니다: "+previous);}status("설치 완료");
        }
    ...
    internal static void Repair(string target,Action<string> report)
    {
        ...
        try
        {
            ...
            ExtractDesktopRuntime(target,status);
            CreateDesktopShortcut(target,status);
            File.WriteAllText(Path.Combine(target,"VERSION"),"7.2.4");File.WriteAllLines(Path.Combine(target,"repair.log"),log.ToArray());status("복구 완료");
        }
    ...
    private static void Download(string url,string path,string name,long expected,Action<string> status)
    {
        ...
        request.UserAgent="LAKIS-Installer/7.2.4";
    ...
    private static void DownloadParallel(string url,string path,string name,long expected,Action<string> status)
    {
        ...
        for(int index=0;index<segmentCount;index++)
        {
            ...
            tasks[segment]=Task.Run(()=>{
                var request=(HttpWebRequest)WebRequest.Create(url);request.UserAgent="LAKIS-Installer/7.2.4";request.AllowAutoRedirect=true;request.Timeout=30000;request.ReadWriteTimeout=30000;request.AddRange(start,end);
                using(var response=(HttpWebResponse)request.GetResponse())
                {
