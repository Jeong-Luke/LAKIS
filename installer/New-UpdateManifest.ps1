...
$releaseNotesSource = Get-Content -LiteralPath (Join-Path $repo "RELEASE_NOTES.md") -Raw -Encoding UTF8
$releaseNotes = (($releaseNotesSource -split '(?m)^---\r?$', 2)[0]).Trim().Replace("`n", [Environment]::NewLine)
$manifest = [ordered]@{
    version = $Version
