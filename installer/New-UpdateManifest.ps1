...
$releaseNotes = @"
LAKIS Studio v7.3.5 긴급 패치

구도 설정 저장 및 복원 수정
카메라 좌표와 구도 설정 ON/OFF 상태가 업데이트 또는 재시작 후 초기화되던 문제를 수정했습니다. 캔버스, 휠, 키보드, 슬라이더, 숫자 입력, 프리셋과 초기화 조작이 모두 저장됩니다.

LAKIS DETAIL 배지 수정
LAKIS 최적화를 켠 DETAIL 결과가 FAST로 잘못 표시되던 문제를 수정했습니다. 생성 중, 완료 후, 이력 이미지 재선택 시 모두 LAKIS DETAIL로 표시됩니다.
