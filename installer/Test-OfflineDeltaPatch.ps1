...
if (-not $PatchDirectory) { $PatchDirectory = Join-Path $repo "dist\LAKIS_v7.3.5-offline-patch" }
...
if ((Get-Content -Raw -LiteralPath (Join-Path $testRoot "VERSION")).Trim() -ne "7.3.5") {
    throw "Offline patch did not advance VERSION to 7.3.5."
}
foreach ($relative in $protected) {
    $path = Join-Path $testRoot $relative
