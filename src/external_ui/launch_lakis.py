...
def acquire_launch_mutex():
    """Hold a Windows mutex for the complete desktop/backend lifetime.

    A port probe alone has a race: two launchers can both observe a free port
    before either backend starts.  The mutex closes that gap and also prevents
    duplicate GPU model copies from filling VRAM.
    """
    kernel32 = ctypes.windll.kernel32
    kernel32.CreateMutexW.argtypes = [ctypes.c_void_p, ctypes.c_bool, ctypes.c_wchar_p]
    kernel32.CreateMutexW.restype = ctypes.c_void_p
    handle = kernel32.CreateMutexW(None, True, LAUNCH_MUTEX_NAME)
    if not handle:
...
def main() -> int:
    ...
    if launch_mutex is None:
        show_error("LAKIS가 이미 실행 중입니다. 기존 창을 사용해 주세요.\n\n오류 코드: LKS-RUN-1000")
        return 1
    ...
    try:
        ...
        if state["comfyui_source"] == "launched_by_lakis":
            command = [str(PYTHON), "-s", str(COMFY_MAIN), "--windows-standalone-build",
                       ...
                       "--preview-method", "auto", "--reserve-vram", "1.0",
                       # LAKIS submits a fresh graph for every image. Retaining
                       # prior node outputs consumes several GB while providing
                       # almost no useful cache hits between generations.
                       "--cache-none"]
            total_ram_gb = (
                psutil.virtual_memory().total / (1024 ** 3)
                if psutil is not None else 0
            )
            if total_ram_gb and total_ram_gb <= 24:
                # Pinned host buffers can create severe memory pressure on
                # 16/24 GB systems. Prefer stability over transfer throughput.
                command.append("--disable-pinned-memory")
                state["memory_profile"] = "low_ram"
            else:
                state["memory_profile"] = "standard"
        ...
        for process in (ui_process, comfy_process):
            ...
            try:
                ...
                try:
                    ...
                    pass
        if launch_mutex:
            ctypes.windll.kernel32.ReleaseMutex(launch_mutex)
            ctypes.windll.kernel32.CloseHandle(launch_mutex)
